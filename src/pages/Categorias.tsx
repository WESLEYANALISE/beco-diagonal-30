...
const Categorias = () => {
  ...
  const getCategoryGradient = (index: number) => {
    const gradients = ['from-pink-500 to-rose-500', 'from-blue-500 to-cyan-500', 'from-green-500 to-emerald-500', 'from-yellow-500 to-orange-500', 'from-purple-500 to-violet-500', 'from-red-500 to-pink-500', 'from-indigo-500 to-blue-500', 'from-teal-500 to-green-500'];
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="animate-pulse space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {[1, 2, 3, 4, 5, 6].map(i => <div key={i} className="h-48 bg-white/20 rounded-2xl"></div>)}
            </div>
          </div>
        </div>
      </div>;
      ...
      <section className="px-4 md:px-6 py-0">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
            ...
            return <Card key={category.categoria} className="overflow-hidden hover:shadow-2xl transition-all duration-500 hover:scale-105 bg-white border-0 shadow-lg group cursor-pointer animate-fade-in hover:-translate-y-1" style={{
              animationDelay: `${index * 0.1}s`
                  ...
                  <CardContent className="p-3 md:p-6">
                    <Button className="w-full bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-semibold transition-all duration-300 hover:scale-105 text-xs md:text-sm py-2 md:py-3 shadow-lg hover:shadow-xl" onClick={e => {
                  e.stopPropagation();
